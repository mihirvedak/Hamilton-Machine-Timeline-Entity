...
// Mock data for downtime reasons (sorted descending)
const downtimeData = [
  { name: "MACHINE BREAKDOWN", value: 145 },
  { name: "MOLD BREAKDOWN", value: 98 },
  { name: "RAW MATERIAL N/A", value: 67 },
  { name: "OPERATOR N/A", value: 52 },
  { name: "QUALITY ISSUES", value: 45 },
  { name: "PACKING MATERIAL N/A", value: 38 },
  { name: "MACHINE UNDER MAINTENANCE", value: 32 },
  { name: "POWER FAILURE", value: 25 },
  { name: "NO PLANNING", value: 18 },
  { name: "MATERIAL PREHEATING", value: 12 },
];

// Calculate cumulative percentages
const total = downtimeData.reduce((sum, item) => sum + item.value, 0);
let cumulative = 0;
const cumulativeData = downtimeData.map((item) => {
  cumulative += item.value;
  return (cumulative / total) * 100;
});

const DowntimeParetoChart = ({ animationKey = 0 }: DowntimeParetoChartProps) => {
  ...
  const options: Highcharts.Options = {
    chart: {
      ...
      scrollablePlotArea: downtimeData.length > 8 ? {
        minWidth: downtimeData.length * 80,
        scrollPositionX: 0,
      } : undefined,
    },
    title: { text: undefined },
    xAxis: {
      categories: downtimeData.map((d) => d.name),
      labels: {
        rotation: -30,
      },
    },
    ...
    <Card className="h-full bg-card shadow-sm">
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg font-semibold">Downtime Pareto</CardTitle>
          <DropdownMenu>
